...
# Impostazione ottimale: k=3
k = 3

# Generiamo il vocabolario di tutti i possibili 3-meri (4^3 = 64 combinazioni)
basi = ['A', 'C', 'G', 'T']
tutti_i_kmeri = [''.join(p) for p in itertools.product(basi, repeat=k)]
print(f"Vocabolario generato: {len(tutti_i_kmeri)} {k}-meri (da AAA a TTT).")

# PERCORSI DEI FILE
fasta_path = "data/processed/eccdna_disease_detection.body.fa"
metadata_clean_path = "data/processed/eccdna_metadata_CLEAN.tsv"
output_path = f"data/processed/eccdna_kmer_{k}_features.tsv"
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            if line.startswith(">"):
                ...
                # MODIFICA CHIRURGICA: Isoliamo l'ID stringendo il campo prima della prima pipe '|'
